...
const loadData = () => {
    fetch("https://api.spacexdata.com/v5/launches")
        ...
        .then(() => {
            ...
            //Adding image of the patches
        }).then(() => {
            console.log("Image of patch loading...");

            //const image = document.querySelector("#flight-patch-image")

            const list = document.querySelector("#flight-patch-image");
            const imageList = flights.map(flight => {
                return flight.links.patch.small
            });

            imageList.forEach((image) => {
                const patchImage = document.createElement("img")

                // make the image created the image item in the array
                // in the json data we have the url of the image  which is the img src
                patchImage.src = image

                list.appendChild(patchImage);
            })


        })

    //.catch((error) => { console.log( "Can't find image")});
